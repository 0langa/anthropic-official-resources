...
{/* Written from this widget's copy or code by \`node apps/academy/scripts/describe-widgets.ts\`, run at the
    repository root. Run it again after you edit its copy or code. described-from: bd6bdf1b8a3ba924 */}
<Copy k="widget.description">
  Explorer: this widget lets a learner switch between three tabs, each
  describing a pattern of work suited to Claude Cowork. Multi-step shows a
  progress panel illustrating how Cowork breaks a task into stages like
  reading sources, comparing, drafting, formatting, and self-review, running
  them as one delegation. File-based shows a folder-style window where Cowork
  reads existing files and saves a finished output back alongside them, rather
  than only creating new documents. Multi-tool shows connector toggles (Gmail,
  Slack, Microsoft 365) representing how Cowork reaches into the apps someone
  already uses and runs a full sequence across them from a single prompt. Each
  tab pairs a definition with examples, teaching learners to recognize which
  tasks are good candidates to hand off to Cowork.
</Copy>
<Copy k="widget.summary">
  Explorer: tabs let learners browse three patterns of work suited to Claude
  Cowork—multi-step, file-based, and multi-tool—each paired with a definition,
  examples, and a visual illustrating how Cowork handles that pattern.
</Copy>
`;export{e as default};