...
{/* Written from this widget's copy by \`node apps/academy/scripts/describe-widgets.ts\`, run at the
    repository root. Run it again after you edit its copy. described-from: 8d1084fa65f50945 */}
<Copy k="widget.description">
  Film: the user asks Claude's competitive-brief skill to compare their
  company against a competitor, using the competitor's public pages and the
  team's own positioning document. Claude reads the materials, then asks the
  user who the resulting deck is for, since that judgment shapes the scoring.
  Claude returns a summary of what it compared, along with a PowerPoint deck
  and an Excel feature matrix laying out where the company wins, loses, or
  draws against the competitor, each claim tied to a source. The user can also
  schedule the same comparison to run periodically, with Claude drafting an
  update post to a shared channel each time. It illustrates using Claude to
  turn scattered competitor and internal documents into a sourced,
  decision-ready sales asset that stays current.
</Copy>
<Copy k="widget.summary">
  Film: the user asks Claude's competitive-brief skill to compare their
  company against a competitor using public pages and an internal positioning
  document. Claude asks who the deck is for, then returns a sourced sales deck
  and feature matrix, and can run on a schedule.
</Copy>
`;export{e as default};