...
{/* Written from this widget's copy by \`node apps/academy/scripts/describe-widgets.ts\`, run at the
    repository root. Run it again after you edit its copy. described-from: d5c01fc8e0b7566a */}
<Copy k="widget.description">
  Exercise: the learner steps through a simulated support channel's first
  morning with Claude Tag's automatic replies turned on, reviewing three
  messages one at a time and what Claude did in response to each, a case where
  it stayed silent on something the team wanted answered, a case where it
  spoke up when no reply was needed, and a case where it replied in a way that
  didn't match how the team actually handles that situation. For each, the
  learner picks between a few possible fixes, building up a message that asks
  Claude to remember its job in that channel. The exercise's goal is to show
  that the automatic-replies setting only controls whether Claude may speak,
  while what it says, what it skips, and what it does when unsure come from
  explicit guidance the team gives it.
</Copy>
<Copy k="widget.summary">
  Exercise: review what Claude did with three messages in a support channel
  with automatic replies on, and pick fixes that build a message asking Claude
  to remember its job. It teaches that automatic replies control only whether
  Claude may speak, not what it says.
</Copy>
`;export{e as default};