...
{/* Written from this widget's copy by \`node apps/academy/scripts/describe-widgets.ts\`, run at the
    repository root. Run it again after you edit its copy. described-from: ee5fb16bc5fd908c */}
<Copy k="widget.description">
  Film: a use-case walkthrough showing how Claude turns a closed variance
  workbook into a short board-ready slide deck. The user asks Claude, via a
  finance skill, to build headline, drivers, and outlook slides from the
  workbook, matching a prior quarter's deck layout and flagging known one-time
  items. Claude reads the workbook, last quarter's deck, and the one-timers
  note, then pauses to ask the user a judgment call about how to present those
  one-off items before building the slides. Claude returns the deck with every
  figure traceable to its source cell, and shows how the request can be
  scheduled to rebuild automatically when a new workbook arrives. It
  illustrates using connected files and a skill to turn financial data into a
  traceable, repeatable executive presentation.
</Copy>
<Copy k="widget.summary">
  Film: the user asks Claude to turn a closed variance workbook into
  board-ready slides using a finance skill, matching a prior deck's layout.
  Claude reads the source files, pauses to ask about presenting one-time
  items, then delivers a traceable deck and a repeatable schedule.
</Copy>
`;export{e as default};