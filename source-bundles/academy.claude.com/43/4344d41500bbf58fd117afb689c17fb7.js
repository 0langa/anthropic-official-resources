...
{/* Written from this widget's copy by \`node apps/academy/scripts/describe-widgets.ts\`, run at the
    repository root. Run it again after you edit its copy. described-from: ea70a3c8c31750e9 */}
<Copy k="widget.description">
  Film: a demonstration of using Claude to review a law firm's quarterly
  invoices against internal billing guidelines and matter budgets. The user
  asks Claude to flag entries that break the guidelines, list matters drifting
  over budget with the variance and driver, and draft talking points for a
  relationship-partner call. Claude reads the guidelines, budgets, and
  invoices, asks the user how far over budget should trigger a flag, then
  returns a variance spreadsheet naming the driver behind each overage and a
  document with rule-referenced talking points, plus entries it was unsure
  about. It closes by showing how the same review can be scheduled monthly
  across every firm on a panel, teaching how Claude can automate recurring
  spend oversight and surface judgment calls rather than making them silently.
</Copy>
<Copy k="widget.summary">
  Film: the user asks Claude to review a law firm's invoices against billing
  guidelines and matter budgets, flagging off-pattern entries and budget
  drift. Claude asks a clarifying question, then returns a variance sheet and
  talking points, showing how recurring spend review can be scheduled
  automatically.
</Copy>
`;export{e as default};