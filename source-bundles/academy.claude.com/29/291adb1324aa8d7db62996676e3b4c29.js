...
{/* Written from this widget's copy by \`node apps/academy/scripts/describe-widgets.ts\`, run at the
    repository root. Run it again after you edit its copy. described-from: ec283283044ad643 */}
<Copy k="widget.description">
  Exercise: the learner steps through several scenarios describing what Claude
  Code returned after a coding task, such as stopping early with unfinished
  work, finishing with a confident but wrong idea, or adding unrequested
  extras, then decides whether to turn an effort dial, which controls how hard
  the model works, or a model dial, which controls how capable or
  knowledgeable it is, and in which direction to turn it. The first move locks
  in that answer and reveals feedback on whether it was the right dial and
  direction, the right dial but wrong direction, or the wrong dial entirely.
  The goal is to build judgment for diagnosing whether a disappointing result
  stems from insufficient effort, excessive effort, or a knowledge gap that
  only a different model can fix, rather than guessing.
</Copy>
<Copy k="widget.summary">
  Exercise: given a scenario describing what Claude Code returned, turn an
  effort dial or a model dial in the right direction, then get feedback on
  whether you diagnosed an effort problem versus a knowledge problem.
</Copy>
`;export{e as default};