import{Nm as e,jm as t}from"../../../../../content-de-meta-mdrrzgmt.js";var n=t(),r=`# Configuration and multi-file skills
...
`,(0,n.jsxs)(i.li,{children:[(0,n.jsx)(i.strong,{children:(0,n.jsx)(i.code,{children:"allowed-tools"})})," pre-approves the listed tools while the skill is active, so Claude can use them without asking, and it doesn't limit Claude to those tools"]}),`
...
`]}),`
`,(0,n.jsx)(i.p,{children:"A basic skill works with just a name and description, but there are several advanced techniques that can make your skills much more effective in Claude Code. Let's walk through the key fields, best practices for descriptions, tool permissions, and how to structure larger skills."}),`
`,(0,n.jsx)(i.h2,{id:"skill-metadata-fields",children:"Skill Metadata Fields"}),`
`,(0,n.jsxs)(i.p,{children:["The agent skills open standard defines the core fields in the SKILL.md frontmatter, and Claude Code adds a few of its own, such as ",(0,n.jsx)(i.code,{children:"model"}),". Two are required, and the rest are optional:"]}),`
`,(0,n.jsxs)(i.ul,{children:[`
...
`,(0,n.jsxs)(i.li,{children:[(0,n.jsx)(i.strong,{children:"allowed-tools"})," (optional) — Pre-approves the listed tools, so Claude can use them without asking permission while the skill is active."]}),`
...
`,(0,n.jsx)(i.h2,{id:"pre-approving-tools-with-allowed-tools",children:"Pre-approving tools with allowed-tools"}),`
`,(0,n.jsxs)(i.p,{children:["Sometimes a skill relies on the same handful of tools every time it runs, and you don't want to approve each call by hand. The ",(0,n.jsx)(i.code,{children:"allowed-tools"})," field pre-approves the tools you list, so Claude can use them without a permission prompt for the rest of the turn that invoked the skill. The grant clears when you send your next message."]}),`
`,(0,n.jsx)(i.p,{children:(0,n.jsx)(i.img,{src:"./621465b21ab0.png",alt:""})}),`
`,(0,n.jsxs)(i.p,{children:["In this example, the ",(0,n.jsx)(i.code,{children:"allowed-tools"})," field is set to ",(0,n.jsx)(i.code,{children:"Read, Grep, Glob, Bash"}),". When this skill is active, Claude can use those four tools without asking permission. It doesn't lose access to anything else: if the skill's instructions lead Claude to edit or write a file, that goes through your normal permission settings like any other tool call. One caution: listing ",(0,n.jsx)(i.code,{children:"Bash"})," on its own pre-approves shell commands in general, so reserve that for skills you trust, or narrow it to a pattern such as ",(0,n.jsx)(i.code,{children:"Bash(git status *)"}),"."]}),`
...
---
`})}),`
`,(0,n.jsxs)(i.p,{children:["If you omit ",(0,n.jsx)(i.code,{children:"allowed-tools"})," entirely, nothing is pre-approved and Claude uses its normal permission model for every tool. To go the other way and take tools away while a skill is active, use a separate Claude Code frontmatter field, ",(0,n.jsx)(i.code,{children:"disallowed-tools"}),", which removes the tools you list from the set Claude can use."]}),`
...
`,(0,n.jsxs)(i.li,{children:["For a skill your team shares, which tools would you pre-approve with ",(0,n.jsx)(i.code,{children:"allowed-tools"}),", and which would you rather Claude keep asking about?"]}),`
`]}),`
