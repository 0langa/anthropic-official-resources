...
{/* Written from this widget's copy by \`node apps/academy/scripts/describe-widgets.ts\`, run at the
    repository root. Run it again after you edit its copy. described-from: 4a3356d148a3b285 */}
<Copy k="widget.description">
  Explorer: lets the learner pick from a set of example strings and tokenize
  each one, watching it split into colored chips, each labeled with its
  integer ID, followed by a character count compared to a token count and a
  short note explaining the pattern. Examples cover cases like common English
  words, numbers, long or rare words, emoji, typos, names, non-English script,
  contractions, and code. The goal is to build intuition for how tokenization
  differs from splitting text into words or characters: common words often
  become single tokens, while numbers, rare words, emoji, other languages, and
  code can split unexpectedly, which explains certain model quirks like
  difficulty with arithmetic or spelling.
</Copy>
<Copy k="widget.summary">
  Explorer: lets the learner pick example strings and tokenize each one,
  watching it split into colored chips with IDs plus a character-to-token
  count, to build intuition for how tokenization differs from splitting by
  word or letter.
</Copy>
`;export{e as default};