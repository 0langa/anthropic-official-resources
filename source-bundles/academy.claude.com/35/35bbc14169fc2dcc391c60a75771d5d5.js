...
{/* Written from this widget's copy by \`node apps/academy/scripts/describe-widgets.ts\`, run at the
    repository root. Run it again after you edit its copy. described-from: 78a7fae21c943f09 */}
<Copy k="widget.description">
  Film: this plays out a use case for turning a company's brand guidelines
  into a reusable Claude skill. The user points Claude at a working folder
  holding brand guidelines, a voice-and-tone document, a design-token file,
  and a connected design-library and workspace-wiki source, asking it to
  enforce color, typography, spacing, and voice rules and refuse off-brand
  output with a reason. Claude reads all sources, asks the user which source
  should win when documents disagree, then writes a skill file and runs it
  once on a sample request, showing a correction with the broken rule named
  and located. The user can then share the skill workspace-wide. It teaches
  that skills built from existing brand documentation let Claude apply and
  enforce style rules consistently without manual review, deferring to human
  judgment only where sources conflict.
</Copy>
<Copy k="widget.summary">
  Film: the user asks Claude to turn brand guidelines, a voice doc, and design
  tokens into a shared skill. Claude reads the sources, checks which should
  win when they conflict, then writes and runs the skill, correcting an
  off-brand request with the rule named.
</Copy>
`;export{e as default};