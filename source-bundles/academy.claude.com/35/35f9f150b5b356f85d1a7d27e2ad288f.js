...
{/* Written from this widget's copy by \`node apps/academy/scripts/describe-widgets.ts\`, run at the
    repository root. Run it again after you edit its copy. described-from: d892929643cddadf */}
<Copy k="widget.description">
  Film: a walkthrough of asking Claude's Operations plugin to run a vendor
  risk review using a /risk-assessment skill. The user asks Claude to score a
  vendor's SOC 2 report, security questionnaire, DPA and MSA against the
  team's risk framework, with a go/no-go call and cited mitigations. Claude
  reads the framework first, then checks evidence and contract terms against
  each control, asking the user a scoping question where judgment is needed
  rather than guessing. The output is a memo with the score and recommendation
  up top, domain-by-domain scoring, and findings each citing a page, row, or
  clause, plus required mitigations with owners. The film closes by showing
  how the skill can be edited and shared so procurement, security, and legal
  score every vendor consistently.
</Copy>
<Copy k="widget.summary">
  Film: the user asks Claude to score a vendor's security documents and
  contract terms against a risk framework, producing a cited memo with a
  go/no-go call and owned mitigations, then shows sharing the skill across
  teams.
</Copy>
`;export{e as default};