...
{/* Written from this widget's copy by \`node apps/academy/scripts/describe-widgets.ts\`, run at the
    repository root. Run it again after you edit its copy. described-from: ecad0b2670a1286f */}
<Copy k="widget.description">
  Exercise: the learner sorts six real-world requests, shown one at a time in
  a mock Claude composer, deciding for each whether a simple search box could
  handle it or whether it needs Claude working through it as a thinking
  partner. After each pick, an explanation clarifies the reasoning, and some
  items offer an alternate phrasing of the same underlying need to show how
  wording can change which category it falls into. A final side-by-side view
  groups all six requests into the two categories with a tally of how the
  learner's choices matched. The exercise teaches that the deciding factor
  isn't the topic itself but whether the answer depends on the person's own
  situation, draft, or judgment—search handles standalone facts, while a
  thinking partner is needed when personal context shapes the answer.
</Copy>
<Copy k="widget.summary">
  Exercise: the learner sorts six real requests as search-box work or
  thinking-partner work, then reviews explanations and a final tally showing
  how wording can disguise which a request truly needs.
</Copy>
`;export{e as default};