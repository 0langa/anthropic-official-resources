...
{/* Written from this widget's copy by \`node apps/academy/scripts/describe-widgets.ts\`, run at the
    repository root. Run it again after you edit its copy. described-from: 8b286077ad701f3a */}
<Copy k="widget.description">
  Film: this walkthrough shows a user asking Claude, via the Sales plugin's
  pipeline-review skill, to rate a customer account's health using its CRM
  record, support tickets, usage data, NPS responses, and success plan. Claude
  reads each source, checks with the user before making the judgment call on
  the rating, then produces a one-page summary showing each tracked signal
  against its agreed goal, a red/yellow/green call with reasoning, and exactly
  two recommended actions tied to specific signals for that week. The film
  closes by showing how to schedule this to run automatically every week
  across an entire account book, surfacing at-risk accounts first in a single
  digest. It teaches how to turn scattered account signals into a consistent,
  recurring health check with clear, limited next steps.
</Copy>
<Copy k="widget.summary">
  Film: the user asks Claude to rate a customer account's health using CRM,
  support tickets, usage data, NPS, and the success plan. Claude checks with
  the user before calling it, then produces a summary with signals against
  goals and two tied actions, schedulable weekly.
</Copy>
`;export{e as default};