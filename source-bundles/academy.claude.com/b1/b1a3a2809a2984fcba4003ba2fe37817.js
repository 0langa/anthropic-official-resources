...
{/* Written from this widget's copy by \`node apps/academy/scripts/describe-widgets.ts\`, run at the
    repository root. Run it again after you edit its copy. described-from: 0991ebc44eacbfa1 */}
<Copy k="widget.description">
  Film: this plays out a use case where the user asks Claude, through a design
  handoff skill, to turn a flow specification and a component library into a
  clickable HTML prototype, restricted to approved library components only.
  Claude reads the spec, design tokens, Figma frames, and component code
  before building, then pauses to ask the user a design decision where the
  flow requires a judgment call rather than a guess. Claude reports what it
  built and delivers an interactive prototype with working navigation between
  screens and realistic placeholder data. The film also shows scheduling a
  recurring rebuild so the prototype stays aligned with the source spec and
  design files. It illustrates using real components and explicit checkpoints
  to keep prototypes testable and production-accurate.
</Copy>
<Copy k="widget.summary">
  Film: the user asks Claude to build a clickable HTML prototype from a flow
  spec and component library, restricted to approved components. Claude reads
  source files, asks a design judgment question, then delivers a working
  prototype and shows scheduling recurring rebuilds to stay current.
</Copy>
`;export{e as default};