...
{/* Learner-facing copy for Embed1.tsx: a still of the claude.ai chat box
    ...
    Research, Web search, Memory, Document, Presentation, Design, Artifact,
    Effort …) are claude.ai interface labels: match the product's own
    wording in the target language. Model names (Sonnet, Opus, Fable) stay
    in English. The file name, the DOCX tag, the ⌘U shortcut and the
    picker's model names are data in the widget and do not appear here. */}
...
<Copy k="output.docs">Document</Copy>
<Copy k="output.slides">Presentation</Copy>
...
{/* Written from this widget's copy by \`node apps/academy/scripts/describe-widgets.ts\`, run at the
    repository root. Run it again after you edit its copy. described-from: 468c0565829ed69b */}
<Copy k="widget.description">
  Explorer: a framed mock-up of the claude.ai message composer showing a
  worked example prompt that attaches a document and asks Claude to summarize
  decisions, action items, and follow-ups in a short bullet format. A numbered
  legend explains what each part of the prompt gives Claude: context for the
  request, the task itself, the desired format, and the attached material as
  source content. Below that, four selectable controls, Add files, Connect
  your tools, Output, and Model, each open an explanation of what that control
  does: attaching files or folders, connecting outside apps like email or
  calendar so Claude can read and act in them, choosing the output format
  (document, presentation, design, or letting Claude decide), and picking a
  model or effort level for harder versus everyday tasks. The goal is teaching
  how a well-formed first prompt is built and what composer options are
  available before sending it.
</Copy>
<Copy k="widget.summary">
  Explorer: a mock claude.ai composer with a worked example prompt and
  attached file, a legend explaining what each part gives Claude, and
  selectable controls revealing what adding files, connecting tools, choosing
  output, and picking a model do.
</Copy>
`;export{e as default};