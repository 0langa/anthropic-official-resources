...
{/* Written from this widget's copy by \`node apps/academy/scripts/describe-widgets.ts\`, run at the
    repository root. Run it again after you edit its copy. described-from: b2a97aaf053be0b8 */}
<Copy k="widget.description">
  Film: a self-playing walkthrough shows the user asking Claude, via a Human
  Resources plugin's recruiting skill, to synthesize several interviewers'
  scorecards for a job candidate into a debrief brief. Claude reads the
  scorecards, the role's competency rubric, and the interview plan, then lines
  up every rating by competency so agreement and splits are visible, flags
  which signals are well-supported versus anecdotal, notes competencies nobody
  assessed, and closes with open questions for the panel rather than a
  hire/no-hire recommendation. It also shows scheduling the same brief to run
  automatically whenever a candidate's scorecards are complete. The point is
  that Claude organizes panel evidence and surfaces disagreement and gaps,
  while the hiring decision stays with the humans in the room.
</Copy>
<Copy k="widget.summary">
  Film: the user asks Claude, via a recruiting skill, to synthesize
  interviewers' scorecards into a debrief brief showing where the panel
  agrees, splits, and lacks coverage, ending with open questions instead of a
  recommendation.
</Copy>
`;export{e as default};