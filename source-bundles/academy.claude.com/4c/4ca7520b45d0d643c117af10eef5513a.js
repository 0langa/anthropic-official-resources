...
{/* Written from this widget's copy by \`node apps/academy/scripts/describe-widgets.ts\`, run at the
    repository root. Run it again after you edit its copy. described-from: 52f30d329bd89281 */}
<Copy k="widget.description">
  Film: a short walkthrough showing how Claude Cowork finds recurring themes
  across a folder of sales call transcripts. The user points Claude at the
  transcripts folder plus a theme taxonomy, connected to Salesforce and Google
  Drive, and asks for each theme's frequency, a representative quote, and the
  accounts that raised it, written up as a one-page brief. Claude reads the
  transcripts and taxonomy, checks account records, codes each call, and asks
  the user which customer requests should be flagged for the product team
  before producing the brief and a supporting coding spreadsheet. It closes by
  showing how the same process can be scheduled to run automatically whenever
  a new quarter's call folder appears. It teaches how Claude can turn
  unstructured call transcripts into counted, quoted, sourced themes tied to
  accounts, useful for voice-of-customer reporting.
</Copy>
<Copy k="widget.summary">
  Film: the user asks Claude to read a folder of sales call transcripts
  against a theme taxonomy and writes a one-page brief with counts, quotes,
  and accounts per theme. It shows Claude checking CRM records, asking which
  findings matter for product, and scheduling the brief to rerun for future
  quarters.
</Copy>
`;export{e as default};