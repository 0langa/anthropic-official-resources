...
{/* Written from this widget's copy by \`node apps/academy/scripts/describe-widgets.ts\`, run at the
    repository root. Run it again after you edit its copy. described-from: a01c078875974f45 */}
<Copy k="widget.description">
  Exercise: This walks through three mock eval cases for a skill, each built
  from a realistic meeting-recap prompt. For each case, the learner compares
  two Claude outputs side by side, one produced with the skill and one
  without, against a shared set of house-rule checks (things like leading with
  decisions, giving every action item an owner and date, surfacing open
  questions, and staying concise). The learner picks which output they'd
  actually send and writes one line of feedback for Claude. It reinforces that
  an eval is just a realistic prompt, a side-by-side comparison against clear
  rules, and specific feedback that Claude can use to revise the skill.
</Copy>
<Copy k="widget.summary">
  Exercise: For three mock meeting-recap prompts, the learner compares a
  with-skill and without-skill output against house-rule checks, picks which
  they'd send, and writes feedback for Claude to revise.
</Copy>
`;export{e as default};