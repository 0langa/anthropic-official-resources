...
{/* Written from this widget's copy by \`node apps/academy/scripts/describe-widgets.ts\`, run at the
    repository root. Run it again after you edit its copy. described-from: 4223b39065ee6694 */}
<Copy k="widget.description">
  Film: a self-playing walkthrough showing how to use Claude to check what a
  new regulation requires of a company's products and policies. The user asks
  Claude, via a regulatory-legal skill, to read the rule text against product
  descriptions and current policies and draft a compliance brief covering what
  changed, which provisions apply and why, and what to do by when. Claude
  reads the regulation, the product descriptions, and the policy bundle,
  checks an internal systems inventory, verifies application dates against the
  official legal source, then pauses to ask the user a scoping question before
  mapping requirements to products. It returns a brief with an applicability
  table, flags for legal review, and a dated action table with owners, and
  shows how the same brief can be scheduled to rerun automatically whenever a
  new rule appears in a watched folder.
</Copy>
<Copy k="widget.summary">
  Film: a self-playing walkthrough where the user asks Claude to check a new
  regulation against product descriptions and policies, producing an
  applicability brief with a dated action table, then shows scheduling the
  same check to rerun automatically for future rules.
</Copy>
`;export{e as default};