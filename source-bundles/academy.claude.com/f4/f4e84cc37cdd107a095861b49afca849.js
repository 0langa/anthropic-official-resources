...
{/* Written from this widget's copy by \`node apps/academy/scripts/describe-widgets.ts\`, run at the
    repository root. Run it again after you edit its copy. described-from: c2dcd8648fc53b36 */}
<Copy k="widget.description">
  Film: a walkthrough showing how to answer an ad-hoc finance question with
  Claude. A stakeholder emails asking why a regional gross margin fell this
  quarter and wants a one-line answer plus a chart for a board pack. The user
  pastes the question into a data-query skill; Claude first reads the schema
  and certified metric definitions, then queries the warehouse, breaking the
  overall decline into its component drivers and highlighting the largest one.
  Claude returns a plain-English explanation, a table of drivers with
  supporting evidence, a chart, and the saved query so the analysis can be
  rerun or audited. The film closes by showing how the same routine can be
  scheduled to run automatically, producing draft answers to new questions for
  review before they're sent. It teaches that Claude can ground ad-hoc
  analysis in a trusted data model, explain variance causes, and produce
  reusable, re-runnable artifacts.
</Copy>
<Copy k="widget.summary">
  Film: the user asks Claude why a regional metric fell, and Claude reads the
  schema and certified definitions, queries the warehouse, and explains the
  drivers with a chart and saved query, then shows the routine scheduled to
  run automatically.
</Copy>
`;export{e as default};