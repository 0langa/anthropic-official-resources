...
{/* Written from this widget's copy by \`node apps/academy/scripts/describe-widgets.ts\`, run at the
    repository root. Run it again after you edit its copy. described-from: 3cf61094e6efd70a */}
<Copy k="widget.description">
  Film: a use-case walkthrough showing how to turn rough engineering notes
  into a design doc or RFC using Claude with a system design skill. The user
  asks Claude to look up prior art and constraints before drafting, using a
  connected wiki and code repository. Claude reads the notes and template,
  searches past proposals and architecture docs, then pauses to ask whether
  the user wants certain rollout sections drafted or left blank, since those
  decisions belong to the user. Claude returns a structured draft with
  multiple approaches, trade-offs, a recommendation, and open questions,
  leaving the user's sections titled but empty. The film closes by showing how
  the same drafting routine can be scheduled to run automatically on new notes
  files, so the team reviews the decision rather than the writing.
</Copy>
<Copy k="widget.summary">
  Film: the user asks Claude to research prior art and constraints, then draft
  a design doc in their template. Claude searches connected wiki and code,
  pauses to ask about sections only the user can decide, then returns a
  structured draft with trade-offs and open questions.
</Copy>
`;export{e as default};