...
{/* Written from this widget's copy by \`node apps/academy/scripts/describe-widgets.ts\`, run at the
    repository root. Run it again after you edit its copy. described-from: 6cceedb5fb1a1312 */}
<Copy k="widget.description">
  Exercise: a mock Cowork setup screen shows connector toggles, a folder
  scope, a task prompt, and a schedule setting for an unattended task about to
  run. The learner selects elements that a careful colleague would question
  before letting it proceed, with explanations appearing for each one found.
  Finding every hazard unlocks a rewrite that turns the same request into a
  safer version, which can be viewed instead of the original via a toggle,
  though not alongside it. The exercise reinforces the lesson's broader theme:
  scoping access and instructions narrowly, writing unambiguous prompts, and
  keeping irreversible or outward-facing actions under deliberate review
  rather than full automation.
</Copy>
<Copy k="widget.summary">
  Exercise: a mock Cowork setup screen lets the learner select elements of a
  task that a careful colleague would question before running it unattended,
  then reveals a safer rewrite to compare against the original.
</Copy>
`;export{e as default};