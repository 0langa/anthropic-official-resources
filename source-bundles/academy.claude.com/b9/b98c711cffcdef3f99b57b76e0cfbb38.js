...
{/* Written from this widget's copy by \`node apps/academy/scripts/describe-widgets.ts\`, run at the
    repository root. Run it again after you edit its copy. described-from: efe27b0b22718011 */}
<Copy k="widget.description">
  Walkthrough: a grid of organizational groups against surfaces (Chat, Claude
  Cowork, Claude Code, Claude for Microsoft 365) steps through a phased
  rollout plan. The learner advances through stages: everything off, then
  broad surfaces turned on for every group, then specialized surfaces added
  only where the work calls for them, and finally one cell that stays blocked
  even though its group otherwise qualifies. Hovering or selecting the blocked
  cell explains that the hold isn't an access restriction but a dependency on
  a separate visibility-reporting decision covered later in the course; a
  toggle lets the learner supply that missing condition and watch the cell
  turn on. It illustrates that surface rollout can be sequenced broadly then
  specialized, and that some grants are gated by constraints from a different
  decision rather than by the access rule itself.
</Copy>
<Copy k="widget.summary">
  Walkthrough: a grid of groups and surfaces steps through a phased rollout,
  turning on broad surfaces first, then specialized ones, until one cell stays
  blocked. A toggle reveals that hold depends on a separate visibility
  decision, not an access rule.
</Copy>
`;export{e as default};