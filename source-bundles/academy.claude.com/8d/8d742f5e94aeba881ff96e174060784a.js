...
{/* Written from this widget's copy by \`node apps/academy/scripts/describe-widgets.ts\`, run at the
    repository root. Run it again after you edit its copy. described-from: 90475f3885226ca8 */}
<Copy k="widget.description">
  Film: a use-case walkthrough showing how to prep for a sales call using
  Claude's call prep skill. The user asks Claude to review a CRM opportunity,
  recent call transcripts, and a shared mutual action plan, then produce a
  one-page prep sheet before a meeting. Claude reads each source in turn,
  pauses to ask the user which asks to prioritize for the call rather than
  guessing, then generates a sheet covering deal status, recurring buyer
  concerns with when they were raised, the asks to make, and likely objections
  paired with responses that worked before. The film closes by showing how
  this can be scheduled to run automatically each morning for every external
  meeting on the calendar, producing a fresh sheet per account.
</Copy>
<Copy k="widget.summary">
  Film: the user asks Claude to prep for a sales call using a CRM record, call
  transcripts, and a shared action plan, producing a one-page sheet with asks
  and objections, then schedules it to run automatically each morning.
</Copy>
`;export{e as default};