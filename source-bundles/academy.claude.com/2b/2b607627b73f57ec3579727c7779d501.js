...
{/* Written from this widget's copy by \`node apps/academy/scripts/describe-widgets.ts\`, run at the
    repository root. Run it again after you edit its copy. described-from: 948e4be08ed999b9 */}
<Copy k="widget.description">
  Film: the user asks Claude, via a Product Management plugin's spec-writing
  skill, to turn a one-page problem statement into a full PRD, naming the
  org's template and a past PRD to match in depth. Claude reads the template,
  the reference document, and a set of logged customer requests and discovery
  notes before asking the user who the first version should serve, along with
  questions about constraints and success metrics. Claude then states what it
  assumed and produces a structured PRD in the template's own sections, with
  goals and non-goals drawn from the user's answers and every open question
  marked with an owner for design review, flagging which answers were
  assumptions. It teaches that Claude interviews for judgment calls rather
  than guessing, and surfaces assumptions explicitly so design review knows
  what to check first.
</Copy>
<Copy k="widget.summary">
  Film: the user asks Claude to turn a one-page problem statement into a full
  PRD using an org template, past PRD, and logged customer requests. Claude
  interviews the user on judgment calls, then delivers a structured PRD with
  assumptions flagged for design review.
</Copy>
`;export{e as default};