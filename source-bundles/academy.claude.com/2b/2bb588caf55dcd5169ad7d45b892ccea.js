...
{/* Written from this widget's copy by \`node apps/academy/scripts/describe-widgets.ts\`, run at the
    repository root. Run it again after you edit its copy. described-from: c2488cf200566038 */}
<Copy k="widget.description">
  Film: a walkthrough showing how to ask Claude to draft performance reviews
  for a manager's reports. The user asks the performance-review skill for a
  draft per report, requiring every claim to be linked to evidence and no
  rating proposed. Claude reads the team's career framework, the prior cycle's
  reviews, and then each report's pull requests, project trackers, and team
  chat threads, before reporting what it read and producing one draft per
  report. The drafts cite sourced evidence for each claim, leave ratings blank
  for the manager to decide, and flag specific places needing the manager's
  own judgment. The film also shows how the same drafting routine can be
  scheduled to run automatically whenever a new review cycle begins, always
  leaving ratings for the manager to fill in.
</Copy>
<Copy k="widget.summary">
  Film: the user asks Claude to draft performance reviews for their reports,
  with every claim linked to evidence and no rating proposed. Claude reads the
  team's framework and each report's work, then returns drafts flagging where
  the manager's own judgment is needed.
</Copy>
`;export{e as default};