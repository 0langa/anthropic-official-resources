...
{/* Written from this widget's copy by \`node apps/academy/scripts/describe-widgets.ts\`, run at the
    repository root. Run it again after you edit its copy. described-from: 2173334817426c33 */}
<Copy k="widget.description">
  Film: a self-playing walkthrough of asking Claude, via the Human Resources
  plugin, to turn a performance framework, rating definitions, review
  template, and cycle instructions into a shared skill that walks managers
  through one employee's review. Claude reads those source documents and the
  cycle calendar, asks the user whether the skill should suggest an overall
  rating, then reports what it built and hands back the skill file plus a test
  draft. The draft quotes rating definitions verbatim, scores only the defined
  competencies, and leaves the overall rating blank for the manager. The film
  closes by showing the skill shared with managers so every review in the
  cycle follows the same framework, definitions, and template consistently.
</Copy>
<Copy k="widget.summary">
  Film: the user asks Claude to turn a performance framework, rating
  definitions, template, and cycle instructions into a shared skill that
  guides managers through one employee's review, then shares it so every
  manager follows the same process.
</Copy>
`;export{e as default};