...
{/* Written from this widget's copy by \`node apps/academy/scripts/describe-widgets.ts\`, run at the
    repository root. Run it again after you edit its copy. described-from: bd746eb74c9a532d */}
<Copy k="widget.description">
  Simulation: a multi-stage walkthrough building a simple Markov chain text
  predictor from scratch. The learner first plays with a phone-style
  autocomplete, then trains a model message by message while watching a
  transition matrix of word-to-word counts fill in, then samples next words
  using the resulting probabilities, then scales up the training data to see
  how vocabulary, contexts, and options grow. A later section lets the learner
  pick an intuitive strategy for choosing among predicted words, then explore
  how temperature, top-k, and top-p sampling parameters formalize that
  instinct. A closing comparison shows that Markov chains and large language
  models both end by sampling from a probability distribution, though LLMs
  compute that distribution very differently and handle far more context, and
  notes this next-word-prediction technique dates back over a century.
</Copy>
<Copy k="widget.summary">
  Simulation: build a Markov chain text predictor step by step, from
  phone-style autocomplete through training a transition matrix, sampling
  words, scaling up data, and tuning temperature, top-k, and top-p, then
  compare the process to how language models sample next tokens.
</Copy>
`;export{e as default};