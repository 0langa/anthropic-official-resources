...
{/* Written from this widget's copy by \`node apps/academy/scripts/describe-widgets.ts\`, run at the
    repository root. Run it again after you edit its copy. described-from: 16e832876a200b25 */}
<Copy k="widget.description">
  Exercise: a base sentence of a request to Claude appears already typed, and
  the learner toggles three sources, each representing a different kind of
  connector, on and off. Turning a source on appends a new sentence to the
  request, asking for something not contained in the original words; turning
  it off removes that sentence again. A running note tracks how many sentences
  are active, and for each connected source explains what outside information
  it lets the request reach. The point is that connectors don't let Claude go
  looking on its own: the learner wrote every sentence themselves, and each
  connection only gave the request somewhere to point to, which disappears
  once that source is disconnected.
</Copy>
<Copy k="widget.summary">
  Exercise: the learner toggles three connector sources on and off, each
  adding a sentence to a request asking for outside information, to show that
  connections only give the request somewhere to point, not a reason for
  Claude to search unprompted.
</Copy>
`;export{e as default};