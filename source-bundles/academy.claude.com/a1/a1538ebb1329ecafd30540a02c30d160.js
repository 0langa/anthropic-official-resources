...
{/* Written from this widget's copy by \`node apps/academy/scripts/describe-widgets.ts\`, run at the
    repository root. Run it again after you edit its copy. described-from: c41480b7e595002f */}
<Copy k="widget.description">
  Film: a self-playing walkthrough of using Claude to explain a monthly budget
  variance. The user points Claude at a close folder holding actuals, the
  prior period's actuals, and a budget file, then asks it to compare the
  current month against both the prior period and budget, and explain any line
  that moved beyond a set threshold for a leadership audience. Claude reads
  the spreadsheets and underlying ledger detail, checks every line against the
  threshold, and drafts a commentary explaining each mover's driver and
  whether it carries into the next month, flagging anything it couldn't
  confirm for human review. The film closes by showing the task scheduled to
  run every Monday, producing a fresh draft whenever new actuals arrive. It
  teaches how connecting source systems and a working folder lets Claude turn
  raw financials into a review-ready variance narrative.
</Copy>
<Copy k="widget.summary">
  Film: a self-playing walkthrough where the user asks Claude to compare
  actuals against the prior period and against budget, explaining lines over a
  set threshold. Claude reads spreadsheets and ledger detail, drafts
  commentary with drivers, flags unclear items, then schedules the task to run
  every Monday.
</Copy>
`;export{e as default};