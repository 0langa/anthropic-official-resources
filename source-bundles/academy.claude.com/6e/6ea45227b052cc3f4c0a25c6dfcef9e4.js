...
{/* Written from this widget's copy by \`node apps/academy/scripts/describe-widgets.ts\`, run at the
    repository root. Run it again after you edit its copy. described-from: 4a68c6cd772dd045 */}
<Copy k="widget.description">
  Film: the user asks Claude, through a design handoff skill, to find every
  existing surface in a product that already uses a filtering pattern similar
  to one being proposed, before designing something new. Claude checks a
  component inventory, then searches a design library, the codebase, and usage
  data to locate surfaces already solving this problem, noting who owns each
  and how it performed. Claude returns a side-by-side audit comparing every
  instance with its owner, component, and usage, then recommends which
  existing pattern to reuse and why the others don't fit. The user can save
  this as a reusable, shareable skill so any team member can repeat the same
  audit by naming a pattern. It teaches that checking for existing solutions
  before designing prevents duplicated, inconsistent patterns across a
  product.
</Copy>
<Copy k="widget.summary">
  Film: the user asks Claude to find every surface already using a similar
  filtering pattern before designing a new one, checking a component
  inventory, library, code, and usage data, then recommending which to reuse.
  The user saves this as a repeatable skill.
</Copy>
`;export{e as default};