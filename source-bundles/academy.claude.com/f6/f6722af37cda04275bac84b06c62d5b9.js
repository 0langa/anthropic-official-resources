...
{/* Written from this widget's copy by \`node apps/academy/scripts/describe-widgets.ts\`, run at the
    repository root. Run it again after you edit its copy. described-from: cd97703565664e2b */}
<Copy k="widget.description">
  Film: a self-playing walkthrough of asking Claude to recap a week's ad
  performance across multiple platforms using a marketing skill. The user
  requests an analysis of what worked, what didn't, and what to change next,
  plus a Slack digest and a saved data sheet. Claude reads each platform's
  export, a conversions file, and a prior recap deck, joins the spend to the
  purchases across sources, then pauses to ask the user which platform's
  purchase count should stand as the single definition going forward. Claude
  returns a summary with a recommended budget reallocation, a recap deck, and
  a joined spreadsheet, then drafts a Slack message for approval before the
  recap is scheduled to run automatically each week. It teaches how Claude can
  reconcile inconsistent metrics across ad platforms into one consistent
  measure, surface budget-shifting recommendations, and automate recurring
  reporting while leaving real decisions for human approval.
</Copy>
<Copy k="widget.summary">
  Film: the user asks Claude to recap weekly ad performance across platforms,
  which Claude reads, pauses to ask about unifying purchase counts, then
  returns a summary, budget recommendation, recap deck, and Slack digest for
  approval, finally scheduling the recap to run automatically.
</Copy>
`;export{e as default};