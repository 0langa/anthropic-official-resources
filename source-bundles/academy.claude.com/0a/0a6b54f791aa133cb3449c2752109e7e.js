...
{/* Written from this widget's copy by \`node apps/academy/scripts/describe-widgets.ts\`, run at the
    repository root. Run it again after you edit its copy. described-from: 8125ef3784ae0b0e */}
<Copy k="widget.description">
  Film: a demonstration of asking Claude, via the Engineering plugin's standup
  skill, to write an on-call handoff brief at the end of a shift. Claude reads
  the previous week's brief, the shift's pages, #support-escalations and
  several incident channels, and production deploys, then stops to ask the
  user whether a carried-over item from last week is fixed or still being
  watched before finishing the draft. The output groups items into incidents,
  CS escalations, and items carried from last week, each marked resolved,
  monitoring, or still open with a one-line cause and a note on what the next
  on-call person should watch, plus a plain list of deploys. Claude drafts a
  channel post that the user must approve before it sends, and the task can be
  scheduled to run automatically at every rotation change, turning scattered
  alert and deploy history into a structured, reviewable handoff.
</Copy>
<Copy k="widget.summary">
  Film: the user asks Claude to draft an on-call handoff from pages, incident
  channels, escalations, and deploys, pausing to confirm whether a
  carried-over item is still being watched before posting the brief for
  approval and scheduling it for future rotations.
</Copy>
`;export{e as default};