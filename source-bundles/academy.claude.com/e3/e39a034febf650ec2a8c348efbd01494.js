...
{/* Written from this widget's copy by \`node apps/academy/scripts/describe-widgets.ts\`, run at the
    repository root. Run it again after you edit its copy. described-from: 20875d027a72f6b0 */}
<Copy k="widget.description">
  Film: the user opens a monthly cloud billing export and asks Claude, via a
  debug skill, to compare costs against the trailing three-month average by
  service and team, flag anything more than 20% over trend, trace each to a
  cause, and assign an owner and action. Claude queries the warehouse for
  historical cost and checks monitoring and code changes for what shifted,
  pausing to ask the user whether one spike was planned before counting it as
  an anomaly. It returns a ranked table showing each over-trend item, the
  amount over trend, the owner, and a next step, alongside a bar chart of
  percent-over-trend against a threshold line. The same check is then
  scheduled to run every Monday, checking for a new month's export. It teaches
  how to turn a cost spike into a structured, ownable investigation rather
  than a manual spreadsheet hunt.
</Copy>
<Copy k="widget.summary">
  Film: the user asks Claude to compare a cloud billing export to trend, flag
  anomalies, and assign owners. Claude checks monitoring and code history,
  confirms one spike with the user, then returns a ranked table with owners
  and next steps, plus a chart, and schedules a weekly check for each new
  month's export.
</Copy>
`;export{e as default};