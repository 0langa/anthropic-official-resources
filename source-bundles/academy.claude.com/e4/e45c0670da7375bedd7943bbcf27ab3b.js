...
{/* Written from this widget's copy by \`node apps/academy/scripts/describe-widgets.ts\`, run at the
    repository root. Run it again after you edit its copy. described-from: 9fa945f3a83bcae5 */}
<Copy k="widget.description">
  Film: a self-playing walkthrough showing how Claude triages a folder of
  incoming NDAs at scale. The user points Claude's review skill at the intake
  folder and the company's NDA playbook, asking it to check each agreement
  against every playbook position and list only the exceptions. Claude reads
  the playbook and each NDA, pauses to ask the user how to handle a clause
  type the playbook doesn't cover, then reports how many agreements are
  cleared for signature, need counsel, or should not be signed, delivering a
  triage spreadsheet and flagging the exact problem clause in one agreement.
  It closes by showing the same check set to run automatically on a recurring
  schedule. The idea: standard NDAs clear without human review, so legal time
  concentrates only on genuine exceptions.
</Copy>
<Copy k="widget.summary">
  Film: the user points Claude at a folder of incoming NDAs and a playbook,
  asking it to flag only exceptions. Claude checks each agreement, asks about
  an uncovered clause, then delivers a triage sheet and sets the check to run
  on a schedule.
</Copy>
`;export{e as default};