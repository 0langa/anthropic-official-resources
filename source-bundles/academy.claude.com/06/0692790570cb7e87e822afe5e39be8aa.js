...
{/* Written from this widget's copy by \`node apps/academy/scripts/describe-widgets.ts\`, run at the
    repository root. Run it again after you edit its copy. described-from: c461730482c042d7 */}
<Copy k="widget.description">
  Explorer: this widget walks through the five blocks of context that get
  prepended before a learner's first message to Claude in an assistant
  product, each sized roughly by its token count. The blocks are a system
  prompt written by the app's developers, user preferences typed once in
  settings, a list of available tools with their descriptions, a short
  manifest of skills Claude can call on if needed, and written memory Claude
  saved from earlier sessions. Each block shows who wrote it, what it's for,
  and a sample excerpt. A closing summary totals the token count across all
  blocks. The goal is to show that a substantial, invisible stack of context
  precedes any typed message, and that its size and composition vary by
  product.
</Copy>
<Copy k="widget.summary">
  Explorer: steps through five blocks of context prepended before a learner's
  first message to Claude, each showing its author, purpose, and an excerpt,
  then totals their token count in a summary.
</Copy>
`;export{e as default};