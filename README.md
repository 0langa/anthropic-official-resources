...
Initial bulk Firecrawl crawls failed with HTTP 429. A final retry after the rate-limit window cleared ended with HTTP 504 (MCP request timed out), without a crawl ID or returned pages. Individual retrieval then reported a 10-request-per-minute account limit. Direct HTTP from the authoring environment returned 403. Consequently thousands of discovered pages remain unpopulated. No missing page has been replaced with an invented summary or placeholder Markdown file.
