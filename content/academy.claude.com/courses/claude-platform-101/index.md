...
You'll send your first request and read the response, then choose the right model for a job (Fable, Opus, Sonnet, or Haiku) and weigh the cost-and-latency trade-off on your own examples instead of guessing. From there you'll build the agent loop by hand to see how Claude acts, observes, and decides, then collapse that hand-written loop with the SDK's Tool Runner once you understand what it does for you. You'll give Claude reach through tool use and let it reason through hard problems with extended thinking.
...
The final section hands work off. You'll learn when to run your own loop and when to let Anthropic run a sandboxed, managed agent for you, then build one end to end, consuming the event stream as Anthropic runs the loop and reports back. A standalone lesson shows you how to build with the API using Claude Code itself, which is also why the rest of the course matters: you need to know what good code looks like to review what an agent writes for you. Most lessons include a code demo you can adapt for your own projects.

## Recommended prerequisites[](https://academy.claude.com/courses/claude-platform-101)

Comfort reading and writing code in at least one language, plus basic command-line familiarity. Most demos use the Python SDK (`anthropic`, installed with pip). The lessons "Your first API call", "What is tool use?" and "Building with Claude Code" use the TypeScript SDK (`@anthropic-ai/sdk`) with Node and npm. You don't need to be an expert in either language, but you should be able to follow along and run a script. You'll need a Claude Console account and an API key from platform.claude.com. New accounts get a small amount of free credit to test the API. After that, you add credit under Settings > Billing. You don't need prior experience building with LLMs.

## Who this is for[](https://academy.claude.com/courses/claude-platform-101)
