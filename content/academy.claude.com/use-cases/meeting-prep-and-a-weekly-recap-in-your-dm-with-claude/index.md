# Meeting prep and a weekly recap in your DM with Claude

In a direct message, set up a routine and Claude Tag posts a prep note before each of your meetings from your own calendar, email and documents, and drafts a Friday recap you can forward.

10 minOperationsClaude Tag

![](https://academy.claude.com/assets/v1/thumbnail.light-lcoz3yje.png)![](https://academy.claude.com/assets/v1/thumbnail.dark-n9q5b4on.png)

In a DM with Claude Tag, Claude reads your own calendar, email and documents, so it can brief you before each meeting and draft your recap on Friday.

Set up a **[routine(opens in new tab)](https://claude.com/docs/claude-tag/users/proactivity)** in the DM by telling Claude once when to post and what to include. Claude then posts a prep note each morning for that day's meetings. Each Friday, it drafts **[the week's recap as a note(opens in new tab)](https://claude.com/docs/claude-tag/users/use-cases/create-artifacts)** you can forward. Only you can read the DM. Claude sends nothing for you, and changes your calendar only when you ask.

## Set up[](https://academy.claude.com/use-cases/meeting-prep-and-a-weekly-recap-in-your-dm-with-claude)

Checklist: what to set up for meeting prep and a weekly recap in a direct message with Claude Tag: the DM itself, and the learner's own calendar, email and document connectors, which are required. Slack threads need nothing connected.



**Ask in the right spot**



[Your DM with Claude(opens in new tab)](https://claude.com/docs/claude-tag/concepts/how-it-works)

The DM uses your own Claude account. Only you can read it, and any routine you set up there is yours.



**Add your own connectors**

![](data:image/svg+xml,%3csvg%20width='48'%20height='48'%20viewBox='0%200%2048%2048'%20fill='none'%20xmlns='http://www.w3.org/2000/svg'%3e%3cpath%20d='M31.3678%2016.6324L24.7365%2015.8956L16.6315%2016.6324L15.8945%2024.0006L16.6313%2031.3688L23.9996%2032.2899L31.3678%2031.3688L32.1046%2023.8165L31.3678%2016.6324Z'%20fill='white'/%3e%3cpath%20d='M19.6541%2028.0627C19.1033%2027.6906%2018.722%2027.1472%2018.5138%2026.4288L19.7922%2025.902C19.9083%2026.3441%2020.1109%2026.6867%2020.4001%2026.9298C20.6875%2027.173%2021.0375%2027.2927%2021.4464%2027.2927C21.8646%2027.2927%2022.2238%2027.1656%2022.524%2026.9114C22.8241%2026.6571%2022.9753%2026.3329%2022.9753%2025.9406C22.9753%2025.5391%2022.8168%2025.2111%2022.5%2024.957C22.1832%2024.7029%2021.7853%2024.5757%2021.3101%2024.5757H20.5714V23.3103H21.2345C21.6434%2023.3103%2021.9879%2023.1998%2022.2679%2022.9787C22.5479%2022.7577%2022.6879%2022.4556%2022.6879%2022.0706C22.6879%2021.728%2022.5626%2021.4553%2022.3121%2021.2509C22.0617%2021.0465%2021.7447%2020.9434%2021.3598%2020.9434C20.984%2020.9434%2020.6855%2021.0429%2020.4645%2021.2436C20.2434%2021.4444%2020.0831%2021.6912%2019.9819%2021.9823L18.7165%2021.4555C18.8841%2020.9802%2019.1918%2020.5602%2019.643%2020.1973C20.0943%2019.8345%2020.6708%2019.652%2021.3708%2019.652C21.8884%2019.652%2022.3544%2019.7516%2022.7671%2019.9523C23.1797%2020.1531%2023.5039%2020.4313%2023.7379%2020.7849C23.9718%2021.1403%2024.0878%2021.5383%2024.0878%2021.9803C24.0878%2022.4316%2023.9792%2022.8129%2023.7618%2023.126C23.5444%2023.4392%2023.2773%2023.6786%2022.9605%2023.8463V23.9218C23.3786%2024.0968%2023.7194%2024.3639%2023.9883%2024.7231C24.2554%2025.0823%2024.3898%2025.5115%2024.3898%2026.0126C24.3898%2026.5136%2024.2627%2026.9612%2024.0085%2027.3536C23.7542%2027.746%2023.4024%2028.0554%2022.9567%2028.2801C22.5091%2028.5048%2022.0063%2028.619%2021.4481%2028.619C20.8016%2028.6208%2020.2048%2028.4348%2019.6541%2028.0627Z'%20fill='%231A73E8'/%3e%3cpath%20d='M27.4998%2021.7203L26.1035%2022.7353L25.4017%2021.6706L27.9198%2019.8543H28.8851V28.4216H27.4998V21.7203Z'%20fill='%231A73E8'/%3e%3cpath%20d='M31.3684%2038.0006L37.9997%2031.3693L34.6841%2029.8958L31.3684%2031.3693L29.8948%2034.685L31.3684%2038.0006Z'%20fill='%23EA4335'/%3e%3cpath%20d='M15.1578%2034.6838L16.6314%2037.9994H31.3677V31.3681H16.6314L15.1578%2034.6838Z'%20fill='%2334A853'/%3e%3cpath%20d='M12.2104%2010C10.9892%2010%2010%2010.9892%2010%2012.2104V31.3676L13.3156%2032.8412L16.6313%2031.3676V16.6313H31.3676L32.8412%2013.3156L31.3678%2010H12.2104Z'%20fill='%234285F4'/%3e%3cpath%20d='M10%2031.3681V35.789C10%2037.0103%2010.9892%2037.9994%2012.2104%2037.9994H16.6313V31.3681H10Z'%20fill='%23188038'/%3e%3cpath%20d='M31.3685%2016.6311V31.3674H37.9998V16.6311L34.6841%2015.1575L31.3685%2016.6311Z'%20fill='%23FBBC04'/%3e%3cpath%20d='M37.9998%2016.6313V12.2104C37.9998%2010.9891%2037.0106%2010%2035.7894%2010H31.3685V16.6313H37.9998Z'%20fill='%231967D2'/%3e%3c/svg%3e)

Google Calendar, Gmail and Google Drive, or Microsoft 365Required

Added to your own Claude account. They apply in the DM and not in channels, and there is nothing for an admin to connect.



Your Slack threads

Nothing to connect. Claude finds your threads with an attendee by searching the workspace's public channels.



**Nothing to invite:** in a direct message every message reaches Claude.

**Check what Claude can reach:** `what can you access here?` Add any missing connector to your own Claude account ([connectors(opens in new tab)](https://claude.com/docs/connectors/overview)).

## What to ask Claude, and what it does[](https://academy.claude.com/use-cases/meeting-prep-and-a-weekly-recap-in-your-dm-with-claude)

Send this once in your DM with Claude. Each morning, Claude posts a prep note in the DM for each meeting on your calendar that day, built from the event, your email and Slack threads with the attendees, and your documents. Ask for whatever you want to know before a meeting.

Film: in a direct message, the user sets up a weekday routine once, and Claude Tag posts a meeting prep note with attendees, open items, documents and a question to ask, citing its calendar, email and document sources.

Claude · direct messageYReplay

Y

YouCopy

@Claude every weekday at 8am Pacific, post prep for each of today's meetings: who, open items, docs, one question.

👀1

ClaudeApp

✱Read your 10:00 event and its attendees from your calendar

○Find your last email with each attendee, and Slack threads with your own team

○List the open items you owe or are owed, and the documents

○Post the prep note here

todos as of just now

ClaudeAppThursday 8:00 AM

Prep for your 10:00, the Alder & Co quarterly review. your calendar

**Attending:** Ines (Alder, finance) and Sam, 10:00 to 10:45.

**Last exchange with Ines:** the export fix, closed on Wednesday after she confirmed by email that the re-run worked. Ines's email, Wed

**Open items:** you owe Ines the SSO timeline you promised on Monday your email to Ines, Mon, and her email about the seat count for the expansion is still unanswered. Ines, seat count

**Docs:** renewal brief v3, the pricing doc and the Q2 usage summary. renewal brief v3 pricing doc Q2 usage summary

**One question to ask:** whether Alder wants the expansion seats in this renewal or quoted separately. renewal brief v3

This is an illustrative example of what Claude Tag can do.

Claude posts the prep notes each morning until you change or stop the routine.

Before you raise an open item in the meeting, open the thread or email the note cites, since the other person may have answered after it posted.

## Follow ups[](https://academy.claude.com/use-cases/meeting-prep-and-a-weekly-recap-in-your-dm-with-claude)

Send these in the same DM. The @-mention is optional there.

### Get a Monday list of deadlines and threads at risk[](https://academy.claude.com/use-cases/meeting-prep-and-a-weekly-recap-in-your-dm-with-claude)

With a second routine, Claude reads your calendar, deadlines and open threads on Monday morning and says which items are at risk and why.

Simulation: a single message to Claude Tag in the user's DM, sometimes with Claude's reply. It sets up a Monday routine or an unsent Friday recap, asks Claude to block calendar time, or asks which routines exist.

Claude · direct messageY

Y

YouCopy

@Claude every Monday at 8am Pacific, post my week: meetings, deadlines, and the two things most likely to slip, with why.

### Have Claude block time on your calendar[](https://academy.claude.com/use-cases/meeting-prep-and-a-weekly-recap-in-your-dm-with-claude)

With your calendar connector on your Claude account, Claude creates an event when you ask, and only then. Here you reply under Monday's post, which listed a security summary due on Wednesday:

Simulation: a single message to Claude Tag in the user's DM, sometimes with Claude's reply. It sets up a Monday routine or an unsent Friday recap, asks Claude to block calendar time, or asks which routines exist.

Claude · direct messageY

Y

YouCopy

Block me an hour Wednesday morning for the summary.

ClaudeAppMonday 8:03 AM

Blocked 9:00 to 10:00 on Wednesday for the security summary.

### Get a Friday recap you can forward[](https://academy.claude.com/use-cases/meeting-prep-and-a-weekly-recap-in-your-dm-with-claude)

Claude drafts the recap in the DM as a note ([artifacts(opens in new tab)](https://claude.com/docs/claude-tag/users/use-cases/create-artifacts)) and does not send it, so you decide whether to forward it and to whom.

Simulation: a single message to Claude Tag in the user's DM, sometimes with Claude's reply. It sets up a Monday routine or an unsent Friday recap, asks Claude to block calendar time, or asks which routines exist.

Claude · direct messageY

Y

YouCopy

@Claude every Friday at 4:30pm Pacific, draft a recap of my week I can forward: what shipped, what slipped and why, threads still waiting on my reply, and next week's first deadlines.

ClaudeAppFriday 4:30 PM

Draft below. I have not sent it to anyone. Forward it, or tell me what to change.



Recap · week of September 14Not sent

in this DM · only you can see it

### See or stop the routines in your DM[](https://academy.claude.com/use-cases/meeting-prep-and-a-weekly-recap-in-your-dm-with-claude)

Claude lists the routines you set up in the DM when asked. To change one, describe the change. To stop one, name it ([manage standing work(opens in new tab)](https://claude.com/docs/claude-tag/users/proactivity)).

Simulation: a single message to Claude Tag in the user's DM, sometimes with Claude's reply. It sets up a Monday routine or an unsent Friday recap, asks Claude to block calendar time, or asks which routines exist.

Claude · direct messageY

Y

YouCopy

@Claude what routines do you have set up here?

## Tips[](https://academy.claude.com/use-cases/meeting-prep-and-a-weekly-recap-in-your-dm-with-claude)

### Direct messages need your own Claude account[](https://academy.claude.com/use-cases/meeting-prep-and-a-weekly-recap-in-your-dm-with-claude)

Direct messages with Claude use a Claude account of your own in the organization, and an Owner can turn them off for everyone. If Claude does not answer in a DM, ask your admin.

### Use a channel of your own when someone else should see the posts[](https://academy.claude.com/use-cases/meeting-prep-and-a-weekly-recap-in-your-dm-with-claude)

The same routines can run in a channel with only you and Claude in it, so people you invite can read the posts ([work from your own channel(opens in new tab)](https://claude.com/docs/claude-tag/users/use-cases/your-own-channel)). To use your own calendar, email or documents there, Claude asks your permission, accesses them as you, and lets you preview what it will post before it appears in the channel.

## Related resources[](https://academy.claude.com/use-cases/meeting-prep-and-a-weekly-recap-in-your-dm-with-claude)

- Learn more in the [Introduction to Claude Tag(opens in new tab)](https://academy.claude.com/courses/introduction-to-claude-tag) course.
- [Get started with Claude Tag(opens in new tab)](https://claude.com/docs/claude-tag/users/getting-started): add @Claude to a channel and see what it can read there.
- [How DMs differ(opens in new tab)](https://claude.com/docs/claude-tag/concepts/agent-identity): the Claude Tag docs on direct messages with Claude.

Was this helpful?
