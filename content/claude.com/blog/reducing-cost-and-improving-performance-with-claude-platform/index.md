...
- Share
  ...
  [Copy link](https://claude.com/blog/reducing-cost-and-improving-performance-with-claude-platform)
...
- **Avoid tool definitions that reorder themselves**. When using the Claude Messages API, the [prompt is assembled](https://platform.claude.com/docs/en/build-with-claude/prompt-caching) in a fixed order with tool definitions rendered at the top. Any change to the tool definition will break the cache.
...
- **Defer rarely used tools**. Declare all your tools up front but mark the rarely used ones [defer\_loading](https://platform.claude.com/docs/en/agents-and-tools/tool-use/tool-use-with-prompt-caching): they stay out of the cached prefix and are appended into the conversation only when Claude looks them up with tool search, so the cache is preserved.

- **Apply system prompt updates as messages**. [Claude Platform](https://platform.claude.com/docs/en/build-with-claude/mid-conversation-system-messages) lets you add a system instruction as a message mid-conversation instead of editing the system prompt, which preserves the cache.
...
- **Move the cache breakpoint as the conversation grows**. With Claude Platform, you can set [automatic caching](https://platform.claude.com/docs/en/build-with-claude/prompt-caching) to automatically apply the cache breakpoint to the last cacheable block.
...
- See our documentation, [here](https://platform.claude.com/docs/en/about-claude/models/optimizing-for-cost-and-intelligence)
- See our cookbook, [here](https://platform.claude.com/cookbook/cost-optimization-cost-optimization)

No items found.

[Prev](https://claude.com/blog/reducing-cost-and-improving-performance-with-claude-platform)Prev

0/5

[Next](https://claude.com/blog/reducing-cost-and-improving-performance-with-claude-platform)Next
...
[Building commerce agents with Claude](https://claude.com/blog/reducing-cost-and-improving-performance-with-claude-platform)Building commerce agents with Claude
...
[A guide to the anatomy of effective commerce agents](https://claude.com/blog/reducing-cost-and-improving-performance-with-claude-platform)A guide to the anatomy of effective commerce agents
...
[How Warp builds self-improving agents on Claude](https://claude.com/blog/reducing-cost-and-improving-performance-with-claude-platform)How Warp builds self-improving agents on Claude
...
[Self-service data analytics in Slack: how Anthropic deploys Claude Tag for ad-hoc questions](https://claude.com/blog/reducing-cost-and-improving-performance-with-claude-platform)Self-service data analytics in Slack: how Anthropic deploys Claude Tag for ad-hoc questions
...
[Subscribe](https://claude.com/blog/reducing-cost-and-improving-performance-with-claude-platform)Subscribe
